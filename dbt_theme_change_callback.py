...
from aio import ThemeSwitchAIO, template_from_url
...
buttons = html.Div(
    [
        dbc.Button("Primary", color="primary", className="mr-1",id="change"),
...
@app.callback(
    Output("graph", "figure") ,Input(ThemeSwitchAIO.ids.store("theme"), "data"),
)
def update_graph_theme(store):
    template = template_from_url(store[0])
...
def change(n):
    return([dbc.themes.QUARTZ, dbc.themes.VAPOR])
